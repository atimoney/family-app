...
const calendarRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/oauth/callback", async (request, reply) => {
    ...
    if (!googleUserId) {
      return reply
        .status(400)
        .send({ error: "Unable to read Google user profile" });
    }
    ...
    if (!refreshToken) {
      return reply
        .status(400)
        .send({ error: "No refresh token returned by Google" });
    }
    ...
    const frontendUrl = (
      fastify.config.FRONTEND_URL ?? "http://localhost:8081"
    ).replace(/\/+$/, "");
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const selectedMap = new Map(
        selectedCalendars.map((sc) => [sc.calendarId, sc]),
      );
  ...
  fastify.put(
    ...
    async (request, reply) => {
      ...
      if (!Array.isArray(calendarIds)) {
        return reply
          .status(400)
          .send({ error: "calendarIds must be an array" });
      }
  ...
  fastify.get(
    "/events",
    { preHandler: [fastify.authenticate] },
    async (request, reply): Promise<CalendarEvent[]> => {
      const userId = request.user?.id as string;
      ...
      if (calendarId) {
        ...
        const selectedCalendars =
          await fastify.prisma.selectedCalendar.findMany({
      ...
      for (const cId of calendarIds) {
        try {
          ...
          const metadataMap = new Map(
            metadata.map((link: { eventId: string; extraData: unknown }) => [
              link.eventId,
              link.extraData as CalendarEventExtraDataV1,
            ]),
          );
        ...
        familyAssignments: extraData?.familyAssignments
          ? {
              ...
              participantFamilyMemberIds:
                extraData.familyAssignments.participantFamilyMemberIds ??
                undefined,
            }
          : undefined,
  ...
  fastify.patch(
    ...
    async (request, reply): Promise<CalendarEvent> => {
      ...
      const needsMove =
        sourceCalendarId && calendarId && sourceCalendarId !== calendarId;
      ...
      if (needsMove) {
        fastify.log.info(
          { eventId, sourceCalendarId, targetCalendarId },
        ...
        try {
          ...
          fastify.log.error(
            { err, eventId, sourceCalendarId, targetCalendarId },
            "Failed to move event",
          );
          return reply
            .status(500)
            .send({ error: "Failed to move event to new calendar" });
        }
      }
      ...
      const event = await updateEvent({
        ...
        event: {
          summary: title,
          description:
            description !== undefined ? (description ?? undefined) : undefined,
          location:
        ...
        familyAssignments:
          extraData?.familyAssignments !== undefined
            ? extraData.familyAssignments
            : undefined,
        // E1: Pass category, audience, tags, and metadata to Google
        category:
          extraData?.category !== undefined ? extraData.category : undefined,
        audience:
          extraData?.audience !== undefined ? extraData.audience : undefined,
        tags: extraData?.tags !== undefined ? extraData.tags : undefined,
        categoryMetadata:
          extraData?.metadata !== undefined ? extraData.metadata : undefined,
      });
      ...
      const startValue =
        event.start?.dateTime ?? event.start?.date ?? start ?? "";
